...
def shift_cal_wcs(mfile, shifts):
    ...
    image_model.write(mfile.replace("_cal.fits", "_fixed_wcs_cal.fits"))
...
def make_sky(
    ...
    for k, cfile in enumerate(files):
        ...
        if istack is None:
            ...
            istack_err = np.empty((isize[0], isize[1], len(files)))
    ...
    bkgr_var = np.nansum(istack_err, axis = 2)/(len(files)**2)

    # turn the sigma clipped std into a variance
    # clip_var = np.square(skyflat_std)
    ...
    if savebkg:
        ...
        fits.writeto(cfile.replace("_cal.fits", "_bkgr_var.fits"), bkgr_var, overwrite = True)
        # fits.writeto(cfile.replace("_cal.fits", "_clip_var.fits"), clip_var, overwrite = True)
