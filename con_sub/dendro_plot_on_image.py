...
fig1 = plt.figure(1, figsize = (8.5,10))
plt.clf()

# plot the continuum filters 
ax1 = fig1.add_subplot(2, 2, 1, projection = w)
im = ax1.imshow(con_rgb, origin = 'lower')

# plot the PAH
ax2 = fig1.add_subplot(2, 2, 2, projection = w)
im = ax2.imshow(pah_rgb, origin = 'lower')

# load and plot the F1500W filter
filepath =  '/Users/etarantino/Documents/JWST_DATA_PAHS/analysis/psf-match/reproject_rot/'
filename = filepath + 'F1500W_reproject_to_F1500W_rot'
F1500W_hdu = fits.open(filename + '.fits')

ax3 = fig1.add_subplot(2, 2, 3, projection = w)
norm = ImageNormalize(F1500W_hdu[0].data, stretch=AsinhStretch(), vmin=-0.02, vmax=0.5)
cmap = sns.color_palette("YlOrBr_r", as_cmap=True)
im = ax3.imshow(F1500W_hdu[0].data, origin = 'lower', cmap = cmap, norm=norm)
...
ax4.scatter(c.ra.deg, c.dec.deg, marker = 'x', c = 'mediumturquoise', transform = ax.get_transform('world'), s = 20, zorder = 0.0001)
