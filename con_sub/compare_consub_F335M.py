...
k2_method_file = consub_path + f'k_method/SextansA_{filt}_k_method_pah_k2.fits'
k2_method_hdu = fits.open(k2_method_file)
k2_method_data = k2_method_hdu[0].data
...
S23_coeff_method_file = consub_path + f'S23_method/SextansA_{filt}_S23_coeff_method_pah.fits'
S23_coeff_method_hdu = fits.open(S23_coeff_method_file)
S23_coeff_method_data = S23_coeff_method_hdu[0].data
...
k2_mask = k2_method_data[mask][ind]
S23_coeff_mask = S23_coeff_method_data[mask][ind]
...
plt.scatter(k_mask, L20_mask, c = 'pink', s = 30, alpha = 0.7, label = 'L20 Method')
# plt.scatter(k_mask, k2_mask, c = 'indigo', s = 30, alpha = 0.7, label = 'k2 = 1.6 Method')
# plt.scatter(k_mask, S23_coeff_mask, c = 'green', s = 30, alpha = 0.7, label = 'S23 coeff')
...
plt.xlabel(filt + ' PAH Flux from k method', size = 'large')
plt.ylabel(filt + ' PAH Flux from other method (see legend)', size = 'large')

plt.xlim(0, 0.2)
plt.ylim(0, 0.2)

savename = 'SextansA_consub_compare_methods_k_method_nok'
...
if filt == 'F335M':
    plt.scatter(k_mask, S23_mask/k_mask, c = 'tomato', s = 30, alpha = 0.7, label = 'S23/k')
    plt.scatter(k_mask, S23_mask/interp_mask, c = 'indigo', s = 30, alpha = 0.7, label = 'S23/interp')
...
### quick scratch test of slopes
F300M_wave = get_pivot_wave('F300M')
F335M_wave = get_pivot_wave('F335M')
F360M_wave = get_pivot_wave('F360M')

wave = (F335M_wave - F300M_wave)/(F360M_wave - F300M_wave)
print(wave)
print(1-wave)

diff = S23_method_data - k2_method_data

save_path = '/Users/etarantino/Documents/JWST_DATA_PAHS/analysis/con_sub/consub_images/compare/'
save_name = f'SextansA_{filt}_method_diff_k_S23_k2.fits'
# fits.writeto(save_path + save_name, diff, header = k2_method_hdu[0].header, overwrite = True)
