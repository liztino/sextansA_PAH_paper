...
def rotate_zoom(pah_data, wcs):
    # filter nans first
    nans = np.isnan(pah_data)
    pah_data[nans] = 0
    
    # box = CustomKernel([[0, 0, 0],
    #                     [0, 1, 0],
    #                     [0, 0, 0]])

    # conv_3 = convolve(pah_data, box, nan_treatment = 'interpolate', preserve_nan = False)
    
    # rotate the data so the aspect ratio is better for the proposal 
    ang = 50.
    rot_data = rotate(pah_data, ang, reshape = False, prefilter = False, order = 2)
    
    ny, nx = pah_data.shape
    center = np.array([nx/2, ny/2])
    
    angle_rad = np.deg2rad(ang)
    cos_angle, sin_angle = np.cos(angle_rad), np.sin(angle_rad)
    
    new_crpix = np.dot([[cos_angle, -sin_angle], [sin_angle, cos_angle]], wcs.wcs.crpix - center) + center
    
    wcs.wcs.crpix = new_crpix
    
    output_data, output_wcs = reproject_interp((rot_data, wcs), wcs, shape_out=rot_data.shape)
        
    return output_data, wcs
...
rot_data, rot_w = rotate_zoom(pah_3['data'], w)


# first make band maps
fig1 = plt.figure(1, figsize = (9,6))
plt.clf()
ax = fig1.add_subplot(projection = rot_w)
# plt.clf()
...
im = ax.imshow(rot_data, origin = 'lower', cmap = 'magma_r', norm = norm)

# plt.figure(4)
# plt.clf()
# plt.imshow(conv_3, origin = 'lower', cmap = 'magma_r', norm = norm)

# norm = simple_norm(pah_3['mask_arr'], 'asinh', min_cut = 0.002, max_cut = np.nanmax(pah_3['mask_arr']))
# im = ax.imshow(pah_3['data'], origin = 'lower', cmap = 'magma_r', norm = norm)

# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.1)

cb = plt.colorbar(im, shrink = 0.63, pad = 0.01)
...
ax.contour(rot_data, levels = levels, colors = 'grey', transform = ax.get_transform(rot_w), linewidths =0.7)
...
savename = 'SextansA_PAH_k_method_F335M_PAH_C4_proposal.pdf'.format(reg_name)
...
ax.contour(rot_data, levels = levels, colors = 'gray', transform = ax.get_transform(rot_w), linewidths =0.7)
...
savename = 'SextansA_PAH_k_method_F770W_PAH_C4_proposal.pdf'
plt.savefig(savepath + savename, bbox_inches='tight',pad_inches = 0.1)
    
# reg_path = '/Users/etarantino/Documents/proposals/JWST_C4/SexA_spec/reg/'
# reg_file = 'regs_for_figure.reg'

# regions = Regions.read(reg_path + reg_file, format='ds9')
# for i, region in enumerate(regions):
#     pix_reg = region.to_pixel(rot_w)
#     pix_reg.plot(ax=ax)
    
# r = Quadrangle((152.7822, -4.7036)*u.deg, 3.2*u.arcsec, 3.7*u.arcsec,
...
rot_data, rot_w = rotate_zoom(pah_11['data'], w)

# first make band maps
fig3 = plt.figure(3, figsize = (9,5))
plt.clf()
ax = fig3.add_subplot(projection = rot_w)
...
ax.contour(rot_data, levels = levels, colors = 'gray', transform = ax.get_transform(w), linewidths =0.7)
...
savename = 'SextansA_PAH_k_method_F1130W_PAH_C4_proposal.pdf'.format(reg_name)
plt.savefig(savepath + savename, bbox_inches='tight',pad_inches = 0.1)
