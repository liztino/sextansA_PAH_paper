...
def plot_D21_mods(U_cut=1, color = 'k'):
    ...
    D21_F335M_table = D21_sp + 'F335M_D21_models_Lindsay_Hands.txt'
    D21_F335M = ascii.read(inpath + D21_F335M_table )  
    print(D21_F335M.colnames)

    D21_F770W_table = D21_sp + 'F770W_D21_models_Lindsay_Hands.txt'
    D21_F770W = ascii.read(inpath + D21_F770W_table )  

    D21_F1130W_table = D21_sp + 'F1130W_D21_models_Lindsay_Hands.txt'
    ...
    # plot the models on top
    plt.scatter(prop_F770W/prop_F1130W, prop_F335M/prop_F1130W, color = color, s = 30, alpha = 0.5)
    plt.plot(prop_F770W/prop_F1130W, prop_F335M/prop_F1130W, color = color, alpha = 0.5)
    for i in range(3):
        plt.plot(prop_F770W[i][:]/prop_F1130W[i][:], prop_F335M[i][:]/prop_F1130W[i][:], color = color, alpha = 0.5)
...
f1 = clumps['F770W_flux']
f2 = clumps['F1000W_flux']
f3 = clumps['F1130W_flux']
...
# keep the 3.3, sub in the 7.7 and 11.3 as Lindsay's
...
anc_path = '/Users/etarantino/Documents/JWST_DATA_PAHS/SEXA_ANCILLARY/'
ha_name = 'MUSE_SexA_pipeline_Halpha.fits'
...
plt.figure(5, figsize = (8, 6))
plt.clf()
plt.errorbar(pah7/pah11, pah3/pah11,
             yerr = err_y, xerr = err_x, fmt = '.', ecolor = 'k', capsize = 2, zorder = 1, markersize = 0.01)
...
# color by Halpha
im = plt.scatter(pah7/pah11, pah3/pah11, c = np.log10(ha_data[ha_y, ha_x]), s = 100)


# # color by Sigma PAH/F1500W in the clump 
# sigma_pah_F1500W = (pah3 + pah7 + pah11)/clumps['F1500W_flux']
# im = plt.scatter(pah7/pah11, pah3/pah11, c = sigma_pah_F1500W, s = 100)

# colorbar label
cb = plt.colorbar(im)
cb.set_label('log(H$\\alpha$)')
...
for i in range(len(clumps)):
    ax.text(pah7[i]/pah11[i], pah3[i]/pah11[i], s = clumps['clump_num'][i])
...
#             s = 150, marker = '^', alpha = 0.5, zorder = 1000, c = 'tomato')


ax.set_xlabel('7.7/11.3 PAH', size = 'x-large')
...
savename = 'testing_band_ratios_3_7_{:s}.png'.format(D21_sp)
...
prop = np.log10(ha_data[ha_y, ha_x])
...
savepath = '/Users/etarantino/Documents/JWST_DATA_PAHS/analysis/con_sub/band_ratios/'
savename = 'testing_band_ratios_prop_Halpha.png'
plt.savefig(savepath + savename, bbox_inches='tight',pad_inches = 0.1, dpi = 300)
