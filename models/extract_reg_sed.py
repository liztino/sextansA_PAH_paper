...
for i, filt in enumerate(filt_list):
    ...
    if filt in miri_filts:
        if filt == 'F1130W':
            infile = miri_path + f'miri_{filt}_stage3_asn_skysub_prop_i2d.fits'
            
        else:
            infile = miri_path + f'miri_{filt}_stage3_asn_fixed_wcs_skysub_prop_i2d.fits'
        ...
        infile = nircam_path + f'jw02391007001_nircam_{filt_name}_asn_modb_bkgrsub_i2d.fits'
        filt_file = '{:s}_mean_system_throughput.txt'.format(filt)
...
names = ['filt', 'mu_low', 'mu_high', 'mu_mid', 'tot_flux', 'avg', 'med', 'std']
