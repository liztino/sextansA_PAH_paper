...
if filt_contam == 'F770W':
    ...
    filt_list = [ 'F770W', 'F444W', 'F560W', 'F300M','F335M', 'F360M']
...
for spec_val in PDR_spec_arr:
    ...
    # ax_list[0].plot(spec['wavelength'], norm_val*filt_dict['F1000W'](spec['wavelength'].value * u.micron), c = 'gray', lw = 1, alpha = 0.5, ls = '-', zorder = 0.1)    
    ...
    k = filt_through_Jy[1]/filt_through_Jy[0]
    
        
    ax_list[0].set_title('{:s} k = F1130W/F1000W = {:3.2f}'.format(spec_val, k))
    ...
    plt.clf()
    # plt.plot(tot.wavelength.to(u.micron), tot.flux, c = 'red', label = 'Total')
    plt.plot(pah.wavelength.to(u.micron), pah.flux, c = 'cornflowerblue', label = 'PAH')
    # plt.plot(con.wavelength.to(u.micron), con.flux, c = 'blue', label = 'Continuum')
    # plt.plot(star.wavelength.to(u.micron), star.flux, c = 'orange', label = 'Stellar')
    
    # plt.plot(spec['wavelength'],  norm_val*filt_dict['F1130W'](spec['wavelength'].value * u.micron), c = 'gray', lw = 1, alpha = 0.5, ls = '-', zorder = 0.1)    
    plt.plot(spec['wavelength'],  norm_val*filt_dict['F1000W'](spec['wavelength'].value * u.micron), c = 'gray', lw = 1, alpha = 0.5, ls = '-', zorder = 0.1) 

    plt.xlabel('Wavelength ($\mu$m)', size = 'x-large')
    plt.ylabel('Flux', size = 'x-large')
    plt.title(f'{spec_val}')
    
    plt.xlim(wave_low, wave_high)

    plt.ylim(0,  norm_val + norm_val*0.2)

    plt.legend(loc='best')
    
    savepath = '/Users/etarantino/Documents/JWST_DATA_PAHS/analysis/models/plots/calc_PAH_contamination/{}_con_fit_tests_v4/'.format(filt_contam)
    savename = 'PAHFIT_{}_PDRs4ALL_{}_spec_decomp_all_filts.pdf'.format(filt_contam, spec_val)
    # plt.savefig(savepath + savename)
